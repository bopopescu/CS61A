...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('improper length of vals vs. formals')
        child = Frame(self)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('{0}: wrong number of arguments'.format(e))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        lambda_rep = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(target.first, lambda_rep)
        return target.first
        # END PROBLEM 9
    else:
